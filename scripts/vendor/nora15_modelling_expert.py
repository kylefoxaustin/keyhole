...
# [keyhole-vendor] removed dead import: import tensorflow as tf
# [keyhole-vendor] removed dead import: import dlimp as dl
import PIL.Image as Image

T = TypeVar("T", bound="VLAWithExpert")

def resize_image(image1):
    # [keyhole-vendor] Original used dlimp/TF (dl.transforms.resize_image, lanczos3
    # + antialias). Replaced with PIL LANCZOS — the antialiased lanczos3 equivalent
    # — to drop the tensorflow/dlimp dependency. Same 224x224 output; the model's
    # processor resizes to 224 again downstream, so this only pre-conditions size.
    if not isinstance(image1, Image.Image):
        image1 = Image.fromarray(np.asarray(image1).astype(np.uint8))
    return image1.resize((224, 224), Image.LANCZOS)
