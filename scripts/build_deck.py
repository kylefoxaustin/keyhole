...
def slide_skippy_recipe_taxonomy(prs: Presentation):
    ...
    rows = [
        ...
        ["Skippy v4 recipe on Gemma 2 9B (cross-family, lift)",
         "Dense 9B, Gemma 2 base, 6/6 reasoning",
         "0.651",  "+3.2pp vs Gemma stock",
         "voice ✓ / safety ✓ / capability ✓",
         "✅ recipe transfers to non-Qwen family with high reasoning floor"],
        ["Skippy v4 recipe on Mistral 7B (cross-family, regress)",
         "Dense 7B, Mistral v0.3 base, 0/6 reasoning",
         "0.568",  "−4.0pp vs Mistral stock",
         "voice ✓ / safety ✓ / capability fails",
         "❌ −1.8σ; low-reasoning base regresses on rag_datasheet/blog/coding — see GOTCHA_7_RESOLUTION.md"],
        ["Skippy v4 recipe on Llama 3.1 8B (cross-family, regress)",
         "Dense 8B, Llama 3.1 base, 1/6 reasoning",
         "0.538",  "−3.2pp vs Llama stock",
         "voice ✓ / safety ✓ / capability fails",
         "❌ −1.3σ; same low-reasoning regression pattern as Mistral"],
    ...
    add_bullet_box(slide, CONTENT_LEFT, 5.4, CONTENT_W, 1.85, [
        ...
        ("• 14B v4 fabricates peripheral parts (made_up_peripheral 3/9). New finding: this is partly Qwen2.5 14B base behavior — v4 recipe inherits and amplifies rather than creating it. Cross-family scoreboard: Qwen 32B / Llama-3.1 8B / Mistral 7B v0.3 all stock at ~6/9 on the same probes BEFORE FT — confident fabrication is a base-model property, not a recipe artifact, and customers can't escape it by switching vendors. Layered mitigation: RAG-grounded refusal data (training-side) + cite-every-claim grounding enforcement (system-side); ship-smaller is the pragmatic dodge.",
         C.TEXT_DIM),
        ("• Recipe transfer is base-capability-coupled (N=5 cross-family, 2026-05-09): clean discriminator is STOCK REASONING FLOOR. 6/6 reasoning bases LIFT (Qwen 7B +3.1pp / Qwen 14B +5.3pp / Gemma 9B +3.2pp); 0-1/6 reasoning bases REGRESS (Mistral 7B −4.0pp / Llama 3.1 8B −3.0pp). Architecture-family hypothesis FALSIFIED at N=5 — Gemma is non-Qwen, no ChatML, lifts. The recipe's re-weighting toward refusal/persona/rag_email comes out of capacity high-reasoning bases have to spare; on low-reasoning bases it spends categories the recipe needs to keep (rag_datasheet, coding, rag_blog). Open question: intermediate-reasoning data point (3-4/6) would falsify-or-confirm whether the cliff is sharp or gradient. Sanctioned framing per personal-ai-framework GOTCHA_7_RESOLUTION.md Addendum.",
         C.ACCENT_INDIGO),
    ], font_size=9)
