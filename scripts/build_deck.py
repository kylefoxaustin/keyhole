...
def slide_skippy_recipe_taxonomy(prs: Presentation):
    ...
    rows = [
        ...
        ["Skippy 14B v4",
         ...
         "⚠️ largest substring lift (+8.7pp vs Qwen 14B base), most evaporative judge (Sonnet ±0.000 / GPT-4o −0.214) — ROBUST under both judges, cleanest substring-vs-judge asymmetry demo; fabricates peripherals"],
        ["Skippy 32B v4 CLEAN",
        ...
        ["Skippy v4 recipe on Gemma 2 9B (cross-family)",
         ...
         "voice ✓ / safety ✓ / capability JUDGE-SENSITIVE",
         "⚠️ Sonnet −0.620 / GPT-4o +0.119 — only judge-disagreement cell across N=5 (faithfulness scoring on RAG-cited responses); two-judge methodology surfaces this — see GOTCHA_7_RESOLUTION.md"],
    ...
    add_bullet_box(slide, CONTENT_LEFT, 5.4, CONTENT_W, 1.85, [
        ...
        ("• 14B v4 fabricates peripheral parts (made_up_peripheral 3/9). New finding: this is partly Qwen2.5 14B base behavior — v4 recipe inherits and amplifies rather than creating it. Cross-family scoreboard: Qwen 32B / Llama-3.1 8B / Mistral 7B v0.3 all stock at ~6/9 on the same probes BEFORE FT — confident fabrication is a base-model property, not a recipe artifact, and customers can't escape it by switching vendors. Layered mitigation: RAG-grounded refusal data (training-side) + cite-every-claim grounding enforcement (system-side); ship-smaller is the pragmatic dodge.",
         C.TEXT_DIM),
        ("• v4 recipe: 9 of 10 judge passes confirm v4 ≤ base across cross-judge corroboration (Sonnet + GPT-4o, 2026-05-10). Substring lifts on Qwen 7B +3.1pp (Sonnet −0.35 / GPT-4o −0.69), Qwen 14B +8.7pp (±0.000 / −0.214), Gemma 9B +3.2pp (−0.620 / +0.119) — Qwen 7B + 14B corroborated by BOTH judges as flat-or-negative; Gemma 9B is judge-sensitive on borderline (the cell where two-judge-minimum methodology earns its keep). Substring regressions on Mistral 7B (−0.218 / −0.048) and Llama 8B (−1.165 / −1.524) corroborated by both judges. Cleanest demonstration: Qwen 14B largest substring lift, most evaporative judge result, ROBUST under both judges. Mechanism: RAG-context faithfulness drops on v4. Substring grader at temp=0 measures FORMAT FIDELITY; trained phrasings match gold tokens but judges see the faithfulness loss. Customer rec: expect voice + refusal to transfer; do NOT expect capability lift on competent bases; run cross-judge for any cell with marginal Δ. Standing methodology: two judges minimum.",
         C.ACCENT_INDIGO),
    ], font_size=9)
