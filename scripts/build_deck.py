...
def slide_skippy_recipe_taxonomy(prs: Presentation):
    ...
    rows = [
        ["Skippy 7B v4 (production)",
         ...
         "✅ SHIP — currently deployed; substring +3.1pp lift erases on judge (Δ −0.350) per N=5 asymmetry test"],
        ["Skippy 14B v4",
         "Dense 14B, Qwen2.5 base, 3/6 reasoning, 9/9 refusal",
         "0.727",  "+2.2pp",
         "voice ✓ / capability caveat",
         "⚠️ largest substring lift (+8.7pp vs Qwen 14B base) but most evaporative judge result (Δ ±0.000) — cleanest substring-vs-judge asymmetry; fabricates peripherals (partly base inheritance)"],
        ["Skippy 32B v4 CLEAN",
        ...
        ["Skippy v4 recipe on Gemma 2 9B (cross-family)",
         ...
         "voice ✓ / safety ✓ / capability NOT corroborated by judge",
         "⚠️ judge Δ −0.620 (substring lift reverses, strongest erasure) — see GOTCHA_7_RESOLUTION.md"],
        ["Skippy v4 recipe on Mistral 7B (cross-family, regress)",
         ...
         "voice ✓ / safety ✓ / capability fails (judge corroborated)",
         "❌ judge Δ −0.218 — regression holds on judge, real capability damage"],
        ["Skippy v4 recipe on Llama 3.1 8B (cross-family, regress)",
         ...
         "voice ✓ / safety ✓ / capability fails (judge corroborated)",
         "❌ judge Δ −1.165 — regression widens on judge (strongest), capability damage confirmed"],
    ...
    add_bullet_box(slide, CONTENT_LEFT, 5.4, CONTENT_W, 1.85, [
        ...
        ("• 14B v4 fabricates peripheral parts (made_up_peripheral 3/9). New finding: this is partly Qwen2.5 14B base behavior — v4 recipe inherits and amplifies rather than creating it. Cross-family scoreboard: Qwen 32B / Llama-3.1 8B / Mistral 7B v0.3 all stock at ~6/9 on the same probes BEFORE FT — confident fabrication is a base-model property, not a recipe artifact, and customers can't escape it by switching vendors. Layered mitigation: RAG-grounded refusal data (training-side) + cite-every-claim grounding enforcement (system-side); ship-smaller is the pragmatic dodge.",
         C.TEXT_DIM),
        ("• v4 recipe produces NO LLM-judge-corroborated capability gain across N=5 cells (2026-05-09, judge: Claude Sonnet semantic rubric on 50-sample held-out subset). Substring lifts on Qwen 7B +3.1pp / Qwen 14B +8.7pp / Gemma 9B +3.2pp ALL erase on judge (Δ −0.350 / ±0.000 / −0.620). Substring regressions on Mistral 7B −3.8pp / Llama 3.1 8B −3.2pp HOLD or widen on judge (Δ −0.218 / −1.165). Cleanest demonstration: Qwen 14B largest substring lift / most evaporative judge result. Mechanism: faithfulness to RAG context drops on v4; trained phrasings still match gold tokens for substring grader, but judge sees the faithfulness loss. The substring grader at temp=0 measures FORMAT FIDELITY, not capability lift. Customer recommendation: expect voice + refusal patterns to transfer reliably; do NOT expect underlying capability lift on bases that already perform competently. Sanctioned framing per personal-ai-framework GOTCHA_7_RESOLUTION.md (judge-corroborated, reviewer-signed-off).",
         C.ACCENT_INDIGO),
    ], font_size=9)
