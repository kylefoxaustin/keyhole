...
def slide_skippy_recipe_taxonomy(prs: Presentation):
    ...
    add_title_subtitle(
        slide,
        "Skippy training campaign — recipes, baselines, and the substring-vs-semantic split",
        "v2-RAG aggregate (132 samples) anchored on Skippy 7B v4 production. Δ-vs-prod column is substring (matches the original campaign); pass-rate column shows substring / semantic where divergent (per 2026-05-11 Qwen-family format-bias finding — see § 8.2 Finding 4 in the briefing).",
    )
    add_pipeline_strip(
        slide,
    ...
    rows = [
        ...
        ["Skippy 14B v4",
         ...
         "0.727 substring / 0.697 semantic",  "+2.2pp",
         "voice ✓ / capability caveat",
         "⚠️ largest substring lift (+8.7pp vs Qwen 14B base), most evaporative judge (Sonnet ±0.000 / GPT-4o −0.214) — ROBUST under both judges, cleanest substring-vs-judge asymmetry demo; fabricates peripherals. Under semantic: +4.8 to +5.5pp vs base — still lifts, one of only 2 cross-family v4 cells surviving semantic regrade."],
        ["Skippy 32B v4 CLEAN",
         "Dense 32B, Qwen2.5 base",
         "0.636 substring / 0.644 semantic",  "−6.9pp",
         "voice ✓ / capability fails",
         "❌ trade not plateau — corpus too small for 32B. Under semantic regrade: vs Qwen 32B base, regression narrows from −4.6pp (substring) to −3.0pp (semantic) — trade direction holds, magnitude smaller."],
        ...
        ["Skippy v4 recipe on Gemma 2 9B (cross-family)",
         "Dense 9B, Gemma 2 base, 6/6 reasoning, 9/9 refusal",
         "0.651",  "+3.2pp substring vs Gemma stock",
         "voice ✓ / safety ✓ / capability JUDGE-SENSITIVE on faithfulness only",
         "⚠️ Sonnet −0.620 / GPT-4o +0.119 — disagreement isolated to RAG-faithfulness scoring (both judges agree on correctness, instruction-following, conciseness); customer rec: characterize faithfulness under multiple judges before deploying — see GOTCHA_7_RESOLUTION.md"],
        ["Skippy v4 recipe on Mistral 7B (cross-family, regress)",
         "Dense 7B, Mistral v0.3 base, 0/6 reasoning",
         "0.568",  "−4.0pp substring vs Mistral stock",
         "voice ✓ / safety ✓ / capability fails (cross-judge corroborated)",
         "❌ Sonnet −0.218 / GPT-4o −0.048 — both judges agree, real capability damage"],
        ["Skippy v4 recipe on Llama 3.1 8B (cross-family, regress)",
         "Dense 8B, Llama 3.1 base, 1/6 reasoning",
        ...
        ["Qwen3-30B-A3B Instruct-2507",
         "MoE, no FT",
         "0.712 substring / 0.657 semantic",  "+0.7pp",
         "(sister base ref — Instruct)",
         "Correct sister baseline for MoE. Under semantic: −5.5pp drop — Qwen-family stock base hit by format-fidelity bias."],
        ["Qwen3-30B-A3B Thinking-2507",
         "MoE thinking variant",
         "0.636 substring / 0.561 semantic",  "−6.9pp",
         "(sister base ref — Thinking)",
         "🔴 wrong sister — confounded prior framing. Under semantic: −12.7pp drop — single largest cell drop in catalog regrade; reinforces Qwen-family-bias finding."],
...
def slide_npu_tier_specs(prs: Presentation):
    ...
    rows = [
        ...
        ["🟢 NPU Mid",           "128-bit LPDDR5X @ 8.4 GT/s","134.4 GB/s","94.08 GB/s (70%)",
         "200 INT8 (no FP)",     "24 GB", "25 W", "37.85 tok/s",   "0.351 s"],
        ["🟡 Mid (+LPDDR5T-11.2)","128-bit LPDDR5T @ 11.2 GT/s","179.2 GB/s","125.44 GB/s (70%)",
         "200 INT8 (no FP)",     "24 GB", "25 W", "50.47 tok/s",   "0.351 s †"],
        ["🟡 Mid (+LPDDR6-12)",  "128-bit LPDDR6 @ 12 GT/s",  "192.0 GB/s","134.40 GB/s (70%)",
         "200 INT8 (no FP)",     "24 GB", "25 W", "54.07 tok/s",   "0.351 s †"],
